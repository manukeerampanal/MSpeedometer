...
// Further work by Manu Mathew Keerampanal - http://manu.co.in/
//
// Project home page: https://github.com/manukeerampanal/MSpeedometer
//
function MSpeedometer(Element) {
    var options = arguments[1] || {};
    var Container = document.getElementById(Element || 'mspeedometer');
    ...
    var CurValue = options.min || 0.0;
    var PointedValue = options.end_point || 0;
    ...
    // Theming
    if (!MSpeedometer.themes['default'])
        throw ('Default theme missing! Please load themes/default.js');
    var theme = MSpeedometer.themes[options.theme] || MSpeedometer.themes['default'];
    for (key in MSpeedometer.themes['default'])
        if (theme[key] == undefined)
            theme[key] = MSpeedometer.themes['default'][key];
    var Color = {
        dial: theme.dial,
        rim: theme.rim,
        rimArc: theme.rimArc,
    ...
    var Canvas = {
        ...
        titletext: TBE.CreateSquareCanvasElement(SizeTitle)
    };
    var Context = {
    ...
    Container.appendChild(Canvas.titletext);
    //
    // Initialization done!
    // Draw everything (still to be refactored)
    //
    this.draw = function() {
        if (Context.background && Context.foreground && Context.hand && Context.titletext) {
            this.drawBackground();
            this.drawCenter();
            this.drawGloss();
            this.drawMeter();
	...
	this.waitToProcess = function() {
		if (animateCallback)
			this.waitToProcess
	}
    this.animatedUpdate = function(value, time, callback) {
        var FPS = 25,
            incr, speedometer = this;
        if (animateCallback)
            this.waitToProcess();//throw ('Animation already running!');
        value = clipValue(value);
        if (isNaN(value) || isNaN(PointedValue))
            throw ('Invalid parameters (value: ' + value + ', time: ' + time + ')');
        if (callback)
            this.addEventListener('speedometer:animateend', callback, false);
        incr = (PointedValue - CurValue) / FPS / (time / 1000);
        animateCallback = function() {

            var done = Math.abs(CurValue) >= Math.abs(PointedValue);
			if(incr < 0) done = Math.abs(CurValue) <= Math.abs(PointedValue);
            if (!animateCallback || done) {
                speedometer.stopAnimation();
                if (done) {
                    speedometer.update(PointedValue);
    ...
    this.drawTitle = function() {
        var cx = Color.titletext.position.x,
            cy = Color.titletext.position.y;
        var title = TitleText;
        var context = Context.titletext;
        context.font = Color.titletext.size + " " + Color.titletext.font;
        var gradient = context.createLinearGradient(0, 0, Position.w, 0);
        gradient.addColorStop("1.0", Color.titletext.color);
